"""Script to run a SimToolReal SHARPA environment with a random-action agent.

Adapted from IsaacLab's scripts/environments/random_agent.py. Project-specific
additions: gym_setup import (registers the simtoolreal_sharpa task) and the
post-CLI apply_object_selection call (so --object overrides take effect).
"""
...
parser = argparse.ArgumentParser(description="Random-action agent for simtoolreal_sharpa.")
...
def main():
    ...
    generator = torch.Generator(device=env.unwrapped.device).manual_seed(args_cli.seed)
