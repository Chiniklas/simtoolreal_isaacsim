from reference.rl_games.rl_games.common import object_factory
from reference.rl_games.rl_games.algos_torch import torch_ext

import torch
import torch.nn as nn

from reference.rl_games.rl_games.algos_torch.d2rl import D2RLNet
from reference.rl_games.rl_games.algos_torch.sac_helper import  SquashedNormal
from reference.rl_games.rl_games.common.layers.recurrent import  GRUWithDones, LSTMWithDones
from reference.rl_games.rl_games.common.layers.value import  TwoHotEncodedValue, DefaultValue
        ...
        def __init__(self, params, **kwargs):
            ...
            if self.is_continuous:
                ...
                if self.fixed_sigma == 'fixed':
                    ...
                    self.sigma = nn.Parameter(torch.zeros(len(self.sigma_ids), actions_num, requires_grad=True, dtype=torch.float32), requires_grad=True)
                else:
            ...
            if self.separate:
                ...
                if self.is_continuous:
                    ...
                    if self.fixed_sigma == 'fixed':
                        ...
                        idxs = (obs_dict['obs'][:,self.sigma_id_idx].reshape(-1,1) == self.sigma_ids).float().argmax(dim=1)
