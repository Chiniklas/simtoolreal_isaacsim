"""Configuration for the SHARPA nut-screw pick-place Isaac Lab task."""
...
def discover_nutscrew_usd_paths() -> dict[str, Path]:
    ...
    for asset_dir in NUTSCREW_ASSET_DIRS:
        if not asset_dir.exists():
            raise FileNotFoundError(f"Missing nut-screw asset folder: {asset_dir}")
...
def configure_cube_object(cfg: "SharpaNutscrewPickPlaceEnvCfg") -> None:
...
def configure_nutscrew_object(cfg: "SharpaNutscrewPickPlaceEnvCfg", asset_name: str) -> None:
...
def configure_multi_nutscrew_objects(cfg: "SharpaNutscrewPickPlaceEnvCfg", asset_names: list[str]) -> None:
...
def apply_object_selection(cfg: "SharpaNutscrewPickPlaceEnvCfg") -> None:
...
@configclass
class SharpaNutscrewPickPlaceEnvCfg(DirectRLEnvCfg):
