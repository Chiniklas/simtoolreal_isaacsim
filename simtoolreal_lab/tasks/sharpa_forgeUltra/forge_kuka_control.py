...
"""forge_kuka: control module.

Merged from the vendored ``factory_control`` module, with the hard-coded
7-DoF Franka arm slices (``[:, 0:7]``) generalized to an explicit
``num_arm_dofs`` and arm-only tensor views. The KUKA iiwa14 arm is also
7-DoF, but the SHARPA finger joints are not guaranteed to occupy a
contiguous trailing slice, so the env gathers the arm-joint columns and
passes arm-width tensors here, then scatters the returned torque back to
the full-width effort buffer.

Imported by the environment. Not directly executed.
"""
...
def compute_dof_torque(
    ...
    """Compute arm DOF torque to move the EE frame towards a target pose.

    Args:
        dof_pos_arm: (num_envs, num_arm_dofs) arm joint positions.
        dof_vel_arm: (num_envs, num_arm_dofs) arm joint velocities.
        jacobian: (num_envs, 6, num_arm_dofs) geometric Jacobian of the EE
            frame restricted to the arm-joint columns.
        arm_mass_matrix: (num_envs, num_arm_dofs, num_arm_dofs) arm-only
            generalized mass matrix block.

    Returns:
        arm_torque: (num_envs, num_arm_dofs) torque for the arm joints only.
        task_wrench: (num_envs, 6) commanded task-space wrench.
    """
    # References:
    ...
    distance_to_default_dof_pos = default_dof_pos_tensor - dof_pos_arm
    distance_to_default_dof_pos = (distance_to_default_dof_pos + math.pi) % (
        2 * math.pi
    ) - math.pi  # normalize to [-pi, pi]
    u_null = cfg.ctrl.kd_null * -dof_vel_arm + cfg.ctrl.kp_null * distance_to_default_dof_pos
    u_null = arm_mass_matrix @ u_null.unsqueeze(-1)
    torque_null = (torch.eye(num_arm_dofs, device=device).unsqueeze(0) - jacobian_T @ j_eef_inv) @ u_null
    arm_torque += torque_null.squeeze(-1)

    arm_torque = torch.clamp(arm_torque, min=-100.0, max=100.0)
    return arm_torque, task_wrench
...
def get_delta_dof_pos(delta_pose, ik_method, jacobian, device):
    """Get delta arm DOF position from delta pose using specified IK method."""
    # References:
