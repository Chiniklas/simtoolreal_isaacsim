...
"""forge_kuka: task configuration module.

Merged from the vendored ``factory_tasks_cfg`` and ``forge_tasks_cfg``
modules into one file. The asset/task definitions are robot-agnostic
(they describe the held/fixed manipulation assets, not the robot), so
they are copied verbatim. The final concrete task classes are renamed
``ForgeKuka*`` to avoid confusion with the Franka ``Forge*`` tasks.
"""
...
# ---------------------------------------------------------------------------
# Forge task mixin + concrete forge_kuka tasks
# ---------------------------------------------------------------------------
@configclass
...
@configclass
class ForgeKukaPegInsert(PegInsert, ForgeTask):
    contact_penalty_scale: float = 0.2


@configclass
class ForgeKukaGearMesh(GearMesh, ForgeTask):
    contact_penalty_scale: float = 0.05


@configclass
class ForgeKukaNutThread(NutThread, ForgeTask):
    contact_penalty_scale: float = 0.05


@configclass
class ForgeKukaNutThreadDeterministic(ForgeKukaNutThread):
    """Stage-A grip/threading variant with all nut+bolt spawn randomization OFF.

    The bolt sits at a fixed pose, the hand servos to a single fixed pose just
    above the bolt tip, and the nut is placed between the fingertips right above
    the bolt - identically every reset. This removes the per-reset pose jitter
    that made the closing fingers knock the (gravity-free) nut around, so we can
    first get a clean, repeatable in-hand hold and train the turn-onto-thread
    motion. Re-introduce randomization later for robustness.
    """

    # Bolt: no position noise; keep the configured base yaw (120 deg) but zero the
    # random spread so the geometry the reach pose was tuned for stays fixed.
    fixed_asset_init_pos_noise: list = [0.0, 0.0, 0.0]
    fixed_asset_init_orn_range_deg: float = 0.0
    # Hand approach: pinch center AT the bolt tip (around the nut on the bolt),
    # not hovering above it. No noise.
    hand_init_pos: list = [0.0, 0.0, 0.0]
    hand_init_pos_noise: list = [0.0, 0.0, 0.0]
    hand_init_orn_noise: list = [0.0, 0.0, 0.0]
    # Nut-in-hand placement: no noise.
